    ...
    return parts.map((part, index) => {
      if (index % 2 === 1) {
        try {
          ...
          return <code key={index} className="text-xs bg-red-500/20 text-red-400 px-1 rounded">{`$${part}$`}</code>;
        }
      }
    ...
    <div className="p-6 md:p-10 max-w-2xl mx-auto flex flex-col gap-8 relative font-sans">
      ...
      {loading && (
        <div className="absolute inset-0 z-50 flex flex-col items-center justify-center gap-6 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-sm rounded-2xl">
          ...
          <p className="text-blue-500/80 animate-pulse font-mono text-sm tracking-widest uppercase">system.generating(quiz)...</p>
        </div>
      )}
      ...
      {mounted && isModalOpen && createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/40 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-[#f8fafc] w-full max-w-md rounded-2xl shadow-xl border border-gray-200 animate-in zoom-in-95 duration-300 flex flex-col relative">
            <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-blue-50/50 rounded-t-2xl">
              <div>
                <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                  <Settings className="w-4 h-4 text-blue-500" /> System Settings
                </h2>
              </div>
              <button onClick={() => setIsModalOpen(false)} className="p-2 text-gray-500 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-4 space-y-3 relative">
              ...
              <div className={`relative ${expandedSection === "examType" ? "z-50" : "z-40"}`}>
                <button 
                  ...
                  className="w-full p-4 flex justify-between items-center bg-white border border-blue-100 rounded-xl hover:bg-gray-50 hover:border-gray-300 shadow-sm transition-colors relative z-50"
                >
                  <div className="flex flex-col items-start">
                    <span className="text-[10px] font-mono text-blue-600 font-semibold uppercase tracking-widest">1. Exam_Type</span>
                    <span className="text-sm font-bold text-gray-800 mt-1">{examType}</span>
                  </div>
                ...
                {expandedSection === "examType" && (
                  <div className="absolute top-full left-0 right-0 mt-2 p-2 flex flex-col gap-1 bg-white border border-gray-200 rounded-xl shadow-lg z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                    {EXAM_TYPES.map((e) => (
                      <button
                        key={e}
                        onClick={() => handleExamTypeChange(e)}
                        className={`px-4 py-3 rounded-lg text-sm font-medium text-left transition-all ${examType === e ? 'bg-blue-50 text-blue-700 font-semibold' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'}`}
                      >
                        {e}
                      </button>
                    ))}
              ...
              <div className={`relative ${expandedSection === "subject" ? "z-50" : "z-30"}`}>
                ...
                >
                  <div className="flex flex-col items-start">
                    <span className="text-[10px] font-mono text-blue-600 font-semibold uppercase tracking-widest">2. Select_Subject</span>
                    <span className={`text-sm font-bold mt-1 ${subject ? 'text-gray-800' : 'text-red-400'}`}>
                ...
                {expandedSection === "subject" && (
                  <div className="absolute top-full left-0 right-0 mt-2 p-2 flex flex-col gap-1 bg-white border border-gray-200 rounded-xl shadow-lg z-50 animate-in fade-in slide-in-from-top-2 duration-200 max-h-64 overflow-y-auto">
                    {EXAM_DATA[examType].map((s) => {
                      ...
                      return (
                        <button
                          ...
                          className={`px-4 py-3 rounded-lg text-sm font-medium text-left transition-all flex justify-between items-center ${subject === s.name ? 'bg-blue-50 text-blue-700 font-semibold' : isLocked ? 'text-gray-400 bg-gray-50 cursor-not-allowed opacity-60' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'}`}
                        >
                          <div className="flex flex-col">
                            <span>{s.name}</span>
                            {isLocked && <span className="text-[9px] text-gray-400 uppercase font-mono mt-0.5">Coming Soon</span>}
                          </div>
              ...
              {/* Category Dropdown (Only for subjects with categories) */}
              {hasCategories && (
                <div className={`relative ${expandedSection === "category" ? "z-50" : "z-20"}`}>
                  ...
                  >
                    <div className="flex flex-col items-start">
                      <span className="text-[10px] font-mono text-blue-600 font-semibold uppercase tracking-widest">3. Select_Category</span>
                      <span className="text-sm font-bold text-gray-800 mt-1">{category}</span>
                    </div>
                    ...
                    <div className="absolute top-full left-0 right-0 mt-2 p-2 flex flex-col gap-1 bg-white border border-gray-200 rounded-xl shadow-lg z-50 animate-in fade-in slide-in-from-top-2 duration-200 max-h-64 overflow-y-auto">
                      {currentSubjectData.categories.map((c) => (
                        <button
                          ...
                          className={`px-4 py-3 rounded-lg text-sm font-medium text-left transition-all ${category === c ? 'bg-blue-50 text-blue-700 font-semibold' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'}`}
                        >
                          {c}
                        </button>
                      ))}
              ...
              {/* Topic Dropdown (Only for subjects with categories) */}
              {hasCategories && (
                <div className={`relative ${expandedSection === "topic" ? "z-50" : "z-10"}`}>
                  ...
                  >
                    <div className="flex flex-col items-start">
                      <span className="text-[10px] font-mono text-blue-600 font-semibold uppercase tracking-widest">4. Select_Topic</span>
                      <span className="text-sm font-bold text-gray-800 mt-1">{topic}</span>
                    </div>
                  ...
                  {expandedSection === "topic" && currentSubjectData?.topics && (
                    <div className="absolute top-full left-0 right-0 mt-2 p-3 flex flex-wrap gap-2 bg-white border border-gray-200 rounded-xl shadow-lg z-50 animate-in fade-in slide-in-from-top-2 duration-200 max-h-64 overflow-y-auto">
                      {currentSubjectData.topics[category].map((t) => (
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-lg text-xs font-mono transition-all border ${topic === t ? 'bg-blue-50 border-blue-200 text-blue-700' : 'bg-transparent border-gray-200 text-gray-500 hover:text-gray-800 hover:bg-gray-100'}`}
                        >
                          {t}
                        </button>
                      ))}
              ...
              <div className={`relative ${expandedSection === "difficulty" ? "z-50" : "z-0"}`}>
                ...
                >
                  <div className="flex flex-col items-start">
                    <span className="text-[10px] font-mono text-blue-600 font-semibold uppercase tracking-widest">Difficulty_Level</span>
                    <span className="text-sm font-bold text-gray-800 mt-1">{difficulty}</span>
                  </div>
                ...
                {expandedSection === "difficulty" && (
                  <div className="absolute bottom-full left-0 right-0 mb-2 p-3 grid grid-cols-3 gap-2 bg-white border border-gray-200 rounded-xl shadow-lg z-50 animate-in fade-in slide-in-from-bottom-2 duration-200">
                    {DIFFICULTIES.map((d) => (
                      <button
                        key={d}
                        onClick={() => {
                        ...
                        className={`py-2 rounded-lg text-xs font-mono transition-all border ${difficulty === d ? 'bg-blue-50 border-blue-200 text-blue-700' : 'bg-transparent border-gray-200 text-gray-500 hover:text-gray-800 hover:bg-gray-100'}`}
                      >
                        {d}
                      </button>
                    ))}
              ...
              {/* AI Model Dropdown */}
              <div className={`relative ${expandedSection === "model" ? "z-50" : "z-0"}`}>
                <button 
                  onClick={() => setExpandedSection(expandedSection === "model" ? "" : "model")}
                  className="w-full p-4 flex justify-between items-center bg-white border border-blue-100 rounded-xl hover:bg-gray-50 hover:border-gray-300 shadow-sm transition-colors relative z-50"
                >
                  <div className="flex flex-col items-start">
                    <span className="text-[10px] font-mono text-blue-600 font-semibold uppercase tracking-widest">AI_Model</span>
                    <span className="text-sm font-bold text-gray-800 mt-1">{MODELS.find(m => m.id === model)?.name || model}</span>
                  </div>
                  <ChevronDown className={`w-5 h-5 text-gray-400 transition-transform ${expandedSection === "model" ? "rotate-180" : ""}`} />
                </button>
                {expandedSection === "model" && (
                  <div className="absolute bottom-full left-0 right-0 mb-2 p-2 flex flex-col gap-1 bg-white border border-gray-200 rounded-xl shadow-lg z-50 animate-in fade-in slide-in-from-bottom-2 duration-200 max-h-64 overflow-y-auto">
                    {MODELS.map((m) => (
                      <button
                        key={m.id}
                        onClick={() => {
                        ...
                        className={`px-4 py-3 rounded-lg text-sm font-medium text-left transition-all ${model === m.id ? 'bg-blue-50 text-blue-700 font-semibold' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'}`}
                      >
                        {m.name}
                      </button>
                    ))}
            ...
            <div className="p-4 bg-blue-50 border-t border-blue-100 mt-auto rounded-b-2xl">
              <button 
                disabled={!subject}
                onClick={() => {
                  if (subject) {
      ...
      <div className="flex items-center justify-between">
        ...
        <button 
          ...
          className={`p-3 rounded-full transition-all border ${
            subject 
              ? 'bg-white/5 dark:bg-[#0a0a0a]/50 border-transparent dark:border-white/5 text-gray-500 dark:text-gray-400 hover:text-blue-500 hover:border-blue-300 hover:shadow-md'
        ...
        <>
          ...
          <div className="flex flex-col gap-3">
            {quiz.content.options.map((option, idx) => {
              ...
              let btnClass = "border-gray-200 dark:border-white/10 hover:border-blue-400 dark:hover:border-cyan-500/50 hover:bg-white/50 dark:hover:bg-cyan-900/10 text-gray-700 dark:text-gray-300 hover:shadow-md";
          ...
          <div className="mt-2 flex flex-col gap-4">
            ...
            {selectedOption !== quiz.content.correct_answer_index && (
              <div className="flex flex-col gap-3">
                {quiz.content.hints.slice(0, hintLevel).map((hint, idx) => (
                  <div key={idx} className="p-4 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/30 rounded-xl flex gap-3 text-amber-900 dark:text-amber-200 shadow-sm">
                ...
                {hintLevel < quiz.content.hints.length && (
                  <button 
                    ...
                    className="self-start px-4 py-2 text-blue-700 dark:text-cyan-400 bg-blue-50/50 dark:bg-cyan-900/20 hover:bg-blue-100 dark:hover:bg-cyan-900/40 border border-blue-200 dark:border-cyan-800/50 rounded-lg text-xs font-mono transition-colors flex items-center gap-2 uppercase tracking-wide"
                  >
            ...
            {selectedOption !== null && (
              <div className={`p-5 rounded-xl border backdrop-blur-md ${selectedOption === quiz.content.correct_answer_index ? 'bg-green-50 dark:bg-green-500/5 border-green-200 dark:border-green-500/20 shadow-sm' : 'bg-gray-50 dark:bg-white/5 border-gray-200 dark:border-white/10'} animate-in fade-in slide-in-from-bottom-4`}>
                ...
                {selectedOption === quiz.content.correct_answer_index && (
                  <button 
                    onClick={fetchNewQuiz}
                    className="mt-6 w-full py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-bold font-mono text-sm transition-all shadow-lg flex justify-center items-center gap-2 uppercase tracking-wide"
                  >
